...
// ✅ CHANGEMENT CLÉ : memoryStorage au lieu de diskStorage
// Les fichiers restent en mémoire (buffer) et sont envoyés à Supabase Storage
const imageFilter = (req, file, cb) => {
  if (file.mimetype.startsWith('image/')) cb(null, true);
  else cb(new Error('Seules les images sont acceptées.'));
};

const uploadAvatar  = multer({ storage: multer.memoryStorage(), limits: { fileSize: 5  * 1024 * 1024 }, fileFilter: imageFilter });
const uploadGallery = multer({ storage: multer.memoryStorage(), limits: { fileSize: 10 * 1024 * 1024 }, fileFilter: imageFilter });
const uploadEvent   = multer({ storage: multer.memoryStorage(), limits: { fileSize: 10 * 1024 * 1024 }, fileFilter: imageFilter });
